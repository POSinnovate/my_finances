...
export function InstallPwaButton({ className, variant = 'compact', onClicked, onRequestModal }: InstallPwaButtonProps = {}) {
  ...
  // If already running as standalone app, hide
  if (isInstalled) {
    return null;
  }
  ...
  return (
    <>
      ...
      {/* Internal modal fallback when not delegating to onRequestModal */}
      {!onRequestModal && (
        <InstallPwaModal
          isOpen={isModalOpen}
