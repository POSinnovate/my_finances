    ...
    <div className="space-y-3.5">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-2.5 sm:gap-3">
        ...
        <div 
          ...
          className="bg-surface-elevated border border-primary/40 hover:border-primary rounded-2xl p-3 sm:p-3.5 shadow-lg transition-all cursor-pointer group active:scale-[0.98] relative overflow-hidden"
          title={
        ...
        >
          <div className="flex items-center justify-between mb-1">
            <span className="text-[10px] sm:text-[11px] font-bold text-foreground/70 flex items-center gap-1">
              <span>Gasto Seguro</span>
              <Sparkles className="w-3 h-3 text-primary opacity-70 group-hover:opacity-100 group-hover:rotate-12 transition-all" />
            </span>
            <Flame className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-primary group-hover:scale-110 transition-transform" />
          </div>
          <p className="text-sm sm:text-base font-black text-foreground truncate group-hover:text-primary transition-colors">
            {formatCOP(safeDaily)}
          </p>
          <span className="text-[9px] sm:text-[10px] text-primary block mt-0.5 font-medium truncate group-hover:underline">
        ...
        {nextExpense ? (
          <div 
            ...
            className="bg-surface-elevated border border-border hover:border-rose-400/80 rounded-2xl p-3 sm:p-3.5 shadow-lg transition-all cursor-pointer group active:scale-[0.98]"
          ...
          <Link 
            href="/budgets"
            className="bg-surface-elevated border border-border hover:border-rose-400/60 rounded-2xl p-3 sm:p-3.5 shadow-lg transition-all block group active:scale-[0.98]"
            title="Sin egresos con fechas programadas. Clic para configurar en Rubros"
          >
            <div className="flex items-center justify-between mb-1">
              <span className="text-[10px] sm:text-[11px] font-bold text-foreground/70">Próximo Egreso</span>
              <ArrowDownRight className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-rose-400/70" />
            </div>
            <p className="text-sm sm:text-base font-bold text-foreground/40 truncate">
              Sin programar
            </p>
        ...
        {nextIncome ? (
          <div 
            ...
            className="bg-surface-elevated border border-border hover:border-accent rounded-2xl p-3 sm:p-3.5 shadow-lg transition-all cursor-pointer group active:scale-[0.98]"
            title="Haz clic para ver opciones de cobro o aplazamiento"
          >
            <div className="flex items-center justify-between mb-1">
              <span className="text-[10px] sm:text-[11px] font-bold text-foreground/70 flex items-center gap-1">
                <span>Próximo Ingreso</span>
                <Info className="w-3 h-3 text-accent opacity-70 group-hover:opacity-100" />
              </span>
            ...
            <span className="text-[9px] sm:text-[10px] text-accent block mt-0.5 truncate font-semibold group-hover:underline">
          ...
          <Link 
            href="/budgets"
            className="bg-surface-elevated border border-border hover:border-amber-400/60 rounded-2xl p-3 sm:p-3.5 shadow-lg transition-all block group active:scale-[0.98]"
            title="Sin ingresos con fechas programadas. Clic para configurar en Rubros"
          >
            <div className="flex items-center justify-between mb-1">
              <span className="text-[10px] sm:text-[11px] font-bold text-foreground/70">Próximo Ingreso</span>
        ...
        <div className="bg-surface-elevated border border-border hover:border-emerald-500/40 rounded-2xl p-3 sm:p-3.5 shadow-lg transition-all">
          <div className="flex items-center justify-between mb-1">
            <span className="text-[10px] sm:text-[11px] font-bold text-foreground/70">Ingreso Mensual</span>
            <TrendingUp className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-emerald-400" />
          </div>
          <p className="text-sm sm:text-base font-black text-emerald-400 truncate">
            {formatCOP(monthlyIncomeWithDates)}
          </p>
          <span className="text-[9px] sm:text-[10px] text-foreground/50 block mt-0.5 truncate" title="Suma de ingresos fijos programados con fechas y quincenas">
            {monthlyIncomeWithDates > 0 ? 'Suma de ingresos con fecha' : 'Sin ingresos con fecha'}
          </span>
        </div>

        {/* 5. Egreso del mes */}
        <div className="bg-surface-elevated border border-border hover:border-rose-500/40 rounded-2xl p-3 sm:p-3.5 shadow-lg transition-all">
          <div className="flex items-center justify-between mb-1">
            <span className="text-[10px] sm:text-[11px] font-bold text-foreground/70">Egresos del Mes</span>
            <ArrowDownRight className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-rose-400" />
          </div>
          <p className="text-sm sm:text-base font-black text-rose-400 truncate">
            {formatCOP(summary.total_spent)}
          </p>
          <span className="text-[9px] sm:text-[10px] text-foreground/50 block mt-0.5 truncate">
            {summary.expense_count} gastos registrados
          </span>
        </div>

        {/* 6. Balance Neto / Ahorro Proyectado */}
        <div className="bg-surface-elevated border border-border hover:border-border/80 rounded-2xl p-3 sm:p-3.5 shadow-lg transition-all">
          <div className="flex items-center justify-between mb-1">
            <span className="text-[10px] sm:text-[11px] font-bold text-foreground/70">Balance Neto</span>
            <Scale className={`w-3.5 h-3.5 sm:w-4 sm:h-4 ${isPositiveBalance ? 'text-primary' : 'text-rose-400'}`} />
          </div>
          <p className={`text-sm sm:text-base font-black truncate ${isPositiveBalance ? 'text-primary' : 'text-rose-400'}`}>
            {isPositiveBalance ? `${formatCOP(projectedNetBalance)}` : formatCOP(projectedNetBalance)}
          </p>
          <span className="text-[9px] sm:text-[10px] text-foreground/50 block mt-0.5 truncate" title="Ingresos con fecha menos los gastos del mes ya ejecutados y los egresos fijos pendientes por registrar">
      ...
      {overdueCommitments.length > 0 && (
        <div className="bg-surface-elevated border border-amber-500/50 rounded-2xl p-4 shadow-xl space-y-3 animate-in fade-in duration-200">
          ...
          <div className="space-y-2.5 pt-1">
            {overdueCommitments.map((item) => {
              ...
              return (
                <div 
                  key={item.id}
                  className={`bg-surface border rounded-xl p-3 flex flex-col sm:flex-row sm:items-center justify-between gap-3 ${
                    isIncome ? 'border-emerald-500/40' : 'border-border'
                  }`}
                >
                  <div className="min-w-0">
                    ...
                    <span className="text-[10px] text-foreground/50 block mt-0.5">
                      {item.daysOverdue === 0 ? (
                        ...
                        <span>
                          Estaba previsto para el <strong className="text-foreground/70">Día {item.dueDay || item.dateStr}</strong> (hace {item.daysOverdue} {item.daysOverdue === 1 ? 'día' : 'días'}).
                        </span>
                      )}
                    </span>
                  </div>
                  ...
                  <div className="flex flex-wrap items-center gap-1.5 shrink-0">
                    ...
                    {onRegisterExpense && (
                      <Button
                        ...
                        className="gap-1 shadow-sm font-extrabold"
                      >
      ...
      {isNextIncomeModalOpen && nextIncome && (
        <Modal
          isOpen={isNextIncomeModalOpen}
          onClose={() => setIsNextIncomeModalOpen(false)}
          title="Detalles del Próximo Ingreso"
          description="Proyección y gestión de cobro"
          icon={<Calendar className="w-5 h-5 text-emerald-400" />}
          maxWidth="lg"
        >
          <div className="space-y-4 pt-1">
            {/* Income Highlight Box */}
            <div className="p-4 rounded-2xl bg-surface-elevated border border-border flex flex-col sm:flex-row sm:items-center justify-between gap-3">
              <div>
                <span className="text-base font-extrabold text-foreground uppercase tracking-wider block text-nowrap truncate">
                  {nextIncome.categoryName || 'Fuente de Ingreso'}
                </span>
                <span className="text-xs text-foreground/70 mt-1 block">
                  Fecha estimada: <strong className="text-foreground">{nextIncome.dateStr}</strong>
                </span>
              </div>

              <div className="text-left sm:text-right">
                <span className="text-xs font-semibold text-foreground/50 block">Monto a recibir:</span>
            ...
            <div className="p-3.5 rounded-2xl bg-surface-elevated/70 border border-border space-y-3">
              {nextIncome.isOverdue ? (
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                  <div>
                    ...
                    <span className="text-[11px] text-foreground/70 block mt-0.5">
      ...
      {nextExpense && (
        <Modal
          isOpen={isNextExpenseModalOpen}
          onClose={() => setIsNextExpenseModalOpen(false)}
          title="Detalles del Próximo Egreso"
          description="Compromiso de pago programado"
          size="md"
        >
          <div className="space-y-4">
            ...
            <div className="p-4 rounded-2xl bg-surface-elevated border border-border flex flex-col sm:flex-row sm:items-center justify-between gap-3">
              <div>
                ...
                <span className="text-xs text-slate-300 mt-1 block">
                  Fecha estimada: <strong className="text-foreground">{nextExpense.dateStr}</strong>
                </span>
              </div>
            ...
            {/* Actions according to date logic */}
            <div className="p-3.5 rounded-2xl bg-surface-elevated/50 border border-border space-y-3">
              {nextExpense.isOverdue ? (
                <div className="space-y-2.5">
                  <div className="text-xs text-amber-300 flex items-center gap-1.5 font-medium">
                    <AlertTriangle className="w-4 h-4 shrink-0 text-amber-400" />
